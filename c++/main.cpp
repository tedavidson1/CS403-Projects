...
int main(int argc, char *argv[])
{
	...
	string op="ge"; //just some standardized value as placeholder
	double threshold=0;
	if (argc==5) {
		op=string(argv[3]);
        string s(argv[4]); //he's just converting something to a double through a stream
...

	{
		...
		if (action=="count") {
			...
			// cout << "min=" << min << endl;
			for (unsigned int i=0; i<employees->size(); i++) { //finds the min
				double amount=employees->at(i)->getEarning();
			...
			// cout << "min=" << min << endl;
			printEmployees(employees, "eq", min); //calls to print employees of this min
		}
    	else if (action=="max") {
            double max=-1;
            for (unsigned int i=0; i<employees->size(); i++) {
